...
# invoking `docker build` directly, e.g. --build-arg MODEL_EXPERIMENT=89.
...
# just say "not found", and BuildKit can run an unrelated validation RUN step
# in parallel with it, so the friendly error isn't guaranteed to win the race.
# classes.json is optional: older experiments (trained before it existed)
# simply won't have one, and src/api.py falls back to CIFAR10_CLASSES then.
